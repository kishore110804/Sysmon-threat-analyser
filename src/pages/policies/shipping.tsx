
import Navbar from '@/components/navbar';
import Footer from '@/components/footer';

export default function ShippingPolicy() {
  return (
    <>
      <Navbar />
      <section className="container mx-auto py-16 px-4">
        <h1 className="text-3xl font-extrabold font-heading mb-6">Shipping Policy</h1>
        <div className="prose max-w-none">
          <p className="text-lg mb-4">Last updated: {new Date().toLocaleDateString()}</p>
          
          <h2 className="text-xl font-bold mt-8 mb-4">1. Processing Time</h2>
          <p>Orders are typically processed within 1-2 business days.</p>
          
          <h2 className="text-xl font-bold mt-8 mb-4">2. Shipping Methods</h2>
          <p>We offer standard and express shipping options.</p>
          
          {/* Add more shipping policy content here */}
        </div>
      </section>
      <Footer />
    </>
  );
}
